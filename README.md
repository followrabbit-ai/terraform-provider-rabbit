...
are rejected by the backend. The built-in admin group can be imported
read-only (e.g. for drift visibility), but membership changes still need
to flow through the Rabbit UI until a future backend fix lifts the
restriction on full-PUT requests for the admin group itself.
